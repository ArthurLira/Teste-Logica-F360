...
ordemKlingon = "kbwrqdnfxjmlvhtcgzps"

def Pergunta1e2(texto):
    
    #Separa as palavras do texto em um array
    ...
    for palavra in palavras:
        ...
        #Verfica se a palavra tem 3 letras, se não encontra letra foo no final e se não encontra letra 'd' na palavra
        if len(palavra) == 3 and foo.find(ultimaLetra) < 0 and palavra.find('d') < 0: 
            preposicoes +=1
        
        #Verfica se a palavra tem 8 letras ou mais e se encontra uma letra foo no final
        if len(palavra) >= 8 and foo.find(ultimaLetra) >= 0 :
            verbos +=1
            
            #Verifica se a primeira letra do verbo é bar
            if foo.find(palavra[0]) < 0:
                verbos1a +=1

    print(f'Nesse texto exsitem {preposicoes} preposições, {verbos} verbos e {verbos1a} verbos em primeira pessoa')

def Pergunta3(texto):
    
    #Cria um dicionario com palavras sem repetição
    palavrasUnicas = set(texto.split())

    dicionarioKlingon = {}

    ordemAlfabetica = "abcdefghijklmnopqrstuvwxyz"
    ...
    for palavra in palavrasUnicas:
        ...
        #Encontra a letra na ordem alfabetica, que equivale a letra de mesmo peso na ordem alfabetica Klingon
        #A tradução é a soma dessas letras, ou seja, uma 'palavra traduzida' em questão de ordem alfabetica
        for letra in palavra:
            traducao += ordemAlfabetica[ordemKlingon.find(letra)]
        
        #Adiciona no dicionario a palavra Kllingon como chave e sua tradução como valor
        dicionarioKlingon[palavra] = traducao
    
    #Ordena e guarda as palavras Klingom em ordem pelas suas traduções
    for palavra in sorted(dicionarioKlingon, key = dicionarioKlingon.get):
        palavrasOrdenado += palavra + ' '
    
    print(f'A ordem alfabetica Klingon deste texto é:\n {palavrasOrdenado}')
...
def Pergunta4(texto):
    ...
    for palavra in palavras:
        ...
        #Atribui valor a cada letra baseado em sua ordem alfabetica e a sequencia de base 20
        for letra in palavra:
            valor += ordemKlingon.find(letra) * base
            base *= 20

        #Verifica se o valor de cada palavra é maior que 440566 e divisivel por 0
        if valor >= 440566 and valor % 3 == 0:
            numBonitos += 1
    
    print(f'Existem {numBonitos} numeros bonitos neste texto')